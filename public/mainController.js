app.controller("mainController", function($scope, $http, $state, dataFromServer) {
    ...
    $scope.addPositionToOrder = function(id, name, count, price) {
        var flag = true;        
        if (count > 0) {
    ...
    $scope.removePositionFromOrder = function(position) {
    ...
    $scope.instantOrder = function() {
        $scope.order = [];
        $scope.basket = [];
        $scope.addPositionToOrder(this.position.id, this.position.name, 1, this.position.price);    
